 		...
 		<form:form action="user/resumen/display.do" enctype="multipart/form-data" method="post" modelAttribute="resumenForm">
              <div class="box box-primary">
                <div class="box-header with-border">
                  <h3 class="box-title">Textos a resumir</h3>
                </div><!-- /.box-header -->
                <!-- form start -->
              ...
              <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                  ...
                  <jstl:if test="${resumenForm.texto.size()>0}">
                 	 <jstl:forEach begin="0" end="${resumenForm.texto.size()-1}" var="val">
							...
							<li><a href="#tab_${val}" data-toggle="tab">Documento ${val}</a></li>
							</jstl:if>
							<jstl:if test="${val==resumenForm.texto.size()-1}">
							
							<li class="active"><a href="#tab_${val}" data-toggle="tab">Documento ${val}</a></li>
							</jstl:if>
						</jstl:forEach>
					</jstl:if> 
					<li> <acme:submit name="add" code="resumen.add" /></li>
                </ul>
                <div class="tab-content">
	                  ...
	                  <jstl:if test="${val!=resumenForm.texto.size()-1}">
	                      ...
	                      <form:input class="form-control" id="name" path="nombre[${val}]"/>
	                      
	                      <label for="title">Texto</label>
	                      
	                      <form:textarea class="form-control" rows="8" id="title" path="texto[${val}]"/>
						  </div><!-- /.tab-pane -->
						</jstl:if>
						<jstl:if test="${val==resumenForm.texto.size()-1}">
          	 ...
          	 <acme:submit name="resumir" code="resumen.detectar"/>
                  </div>
              </div><!-- /.box -->
		</form:form>
