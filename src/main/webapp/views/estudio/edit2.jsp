...
<link rel="stylesheet" href="dist/css/AdminLTE.min.css">
 ...
 <body class="hold-transition skin-blue sidebar-mini">
          ...
          <div class="row">
            ...
            <div class="col-md-12">
 			...
 			<div class="box box-primary">
                ...
                <!-- form start -->
              <form:form action="user/estudio/create2.do" enctype="multipart/form-data" method="post" modelAttribute="estudioForm">
                  <form:hidden path="estudioId"/>
                  
                  <div class="box-body">
                    <div class="form-group">
                      <label>Variables a estudiar</label>
                      <p class="help-block">Marque en color las variables que desea incluir en el estudio, incluyendo el objetivo.</p>
                    	<form:select multiple="true" path="atributosSeleccionados" class="form-control" size="${listaExcel.size()}" id="lista1">
							<form:options items="${listaExcel}" />
						</form:select>
						
                    </div>
                    <div class="form-group">
                      <label>Variable Objetivo</label>
                      <form:select multiple="false" path="label" class="form-control" id="lista2">
						<form:options items="${listaExcel}" />
					  </form:select>
	...
	onclick="javascript: window.location.replace('user/estudio/delete.do?estudioId=${estudioForm.estudioId}')" class="btn btn-primary"/>
                  </div>
				</form:form>
				</div>
...
window.onmousedown = function (e) {
    ...
    if (el.tagName.toLowerCase() == 'option' && el.parentNode.hasAttribute('multiple')) {
        ...
        if (el.hasAttribute('selected')){
            ...
            select2.options[el.index].disabled=true;
            select2.options[el.index].hidden=true;
        }
        else {
        	el.setAttribute('selected', '');
        	select2.options[el.index].disabled=false;
        	select2.options[el.index].hidden=false;
        }
